...
@Component({
  selector: 'app-popup-detalle-plan-adquisicion',
  templateUrl: './popup-detalle-plan-adquisicion.component.html',
  styleUrls: ['./popup-detalle-plan-adquisicion.component.scss'],
})
export class PopupDetallePlanAdquisicionComponent implements OnInit {
  title: string;
  cdp: number;
  instancia: number;
  listaCdp: Cdp[];
        ...
        (documentos: PaginatedResult<Cdp[]>) => {
          ...
          if (this.listaCdp && this.listaCdp.length > 0) {
            ...
            this.alertify.warning(
              'No se pudo obtener detalle del plan anual de adquisiciones para el cdp: ' +
                this.cdp
            );
          }
        },
